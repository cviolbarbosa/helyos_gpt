...
def register_questline_and_create_child_missions(ch, sender, assignment, msg_str, signature):
    ...
    quest_data =   assignment.body.get('missions', [])
    mission_fixed_id =   assignment.body.get('mission_fixed_id', None)
    ...
    try:
        ...
        if operation == 'update_quest':
            ...
            questline = { '_id': questline_id, 'data': quest_data, 'managed_status': QUEST_STATUS.UPDATING}

        if operation == 'failing_quest': # assistant gives up fixing the quest
            mongodb_connector.update_child_mission(mission_fixed_id, managed_status=MANAGED_STATUS.FAILED)
            pass
    
        # The questlien data is the request data for the children as generated by the LLM.
        for mission_request in questline['data']:
            mongodb_connector.create_child_mission(
                                            id='undefined', status=MANAGED_STATUS.WAITING,
                                            parent_quest_id = questline['_id'], 
                                            data = mission_request['data'],                                             
                                            order = mission_request['order'],
        ...
        if mission_fixed_id:
            mongodb_connector.update_child_mission(mission_fixed_id, managed_status=MANAGED_STATUS.CHECKED)
